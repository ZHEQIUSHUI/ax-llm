...
public:
    int init(std::string model_file, std::vector<int> dev_ids)
    {
        ...
        for (int i = 0; i < rank_models.size(); i++)
        {
            ...
            ALOGI("%s init success, ret=%d", rank_filenames[i].c_str(), ret);
        ...
        // {
        //     std::shared_ptr<ax_runner_ax650> runner = std::make_shared<ax_runner_ax650>();
        //     int ret = runner->init(config.model_path.c_str(), config.dev_id);
        //     if (ret != 0)
        //     {
        //         ALOGE("%s init failed, ret=%d", config.model_path.c_str(), ret);
        //         return ret;
        //     }
        //     ALOGI("%s init success, ret=%d", config.model_path.c_str(), ret);
        //     m_runners.push_back(runner);
        //     m_dev_ids.push_back(config.dev_id);
        // }
        return 0;
    }
    void release()
     ...
     * @return int
     */
    int set_input(void *data, int size, int grpid, int idx)
    {
     ...
     * @return int
     */
    int set_input(void *data, int size, int idx)
    {
        return set_input(data, size, 0, idx);
    }
